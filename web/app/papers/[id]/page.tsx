...
import { loadContentIndex, loadWiki } from '@/lib/content'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const entry = index.wikis.find(w => w.id === id)
  if (!entry) return { title: 'Paper not found' }
  return {
    title: `${entry.title} — ConvexPi`,
    description: `Structured wiki for "${entry.title}" covering factor construction, IS/OOS evidence, and practical implications.`,
  }
}
...
export default async function PaperPage({ params }: Props) {
  ...
  const wiki = await loadWiki(id)

  const authorStr = (() => {
    if (!entry.authors?.length) return null
    const names = entry.authors.map((a: { name: string } | string) =>
  ...
  return (
    <div className="container mx-auto px-4 py-10 max-w-3xl">
      ...
      {wiki ? (
        ...
        <div className="rounded-lg border border-border bg-muted/30 px-6 py-10 text-center">
          <p className="text-sm text-muted-foreground">
            Wiki not yet generated for this paper.
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            The weekly pipeline runs every Sunday and will pick this up automatically.
          </p>
        </div>
      )}
      ...
      <footer className="mt-12 pt-4 border-t text-xs text-muted-foreground space-y-1">
        <p>
          Wiki generated by Claude Haiku from the abstract and available full text.
          Not a peer-reviewed summary — verify claims against the original paper.
        </p>
        {entry.updated_at && (
          <p>Last updated: {new Date(entry.updated_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
        )}
      </footer>
    </div>
  )
