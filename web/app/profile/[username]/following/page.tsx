...
export default async function FollowingPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const { data: followingRows } = await supabase
    .from('follows')
    .select('following_id, profiles!follows_following_id_fkey(id, username, display_name, university, github_username)')
  ...
  const rows = (followingRows ?? []) as unknown as FollowingRow[]
  const followingProfiles = rows.map(r => r.profiles).filter(Boolean) as NonNullable<FollowingRow['profiles']>[]
    ...
    <div className="container mx-auto px-4 py-12 max-w-2xl">
      <div className="mb-8">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {followingProfiles.length} {followingProfiles.length === 1 ? 'researcher' : 'researchers'}
        </p>
      </div>

      {followingProfiles.length === 0 ? (
        <p className="text-sm text-muted-foreground py-8 text-center">Not following anyone yet.</p>
      ) : (
